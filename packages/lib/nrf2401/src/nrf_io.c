...
void transfer_read_wbuffer(char target_register, unsigned int length, TransferType_t transferType, char* destination)
{
    char tx_buffer[PackageBufferSize];
    ...
    // if (transferType == RRxPayload)
    // {
    //     char tmp_destination;
    //     spi_write(&dummy, length + 1, &tmp_destination);
    // }
    spi_write(tx_buffer, length + 1, destination);
}
