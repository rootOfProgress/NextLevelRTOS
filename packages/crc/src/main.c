#include "gpio.h"
#include "test.h"
#include "spi.h"
#include "rcc.h"
#include "uart.h"
#include "os_api.h"

#define SV_YIELD_TASK __asm volatile ("mov r6, 2\n" \
                                  "svc 0\n")


#define READ_REGISTER(addr)     (*(volatile unsigned int *) (addr))
#define WRITE_REGISTER(addr, val) ((*(volatile unsigned int *) (addr)) = (unsigned int) (val))
static unsigned char const crc8_table[] = {
    };

unsigned crc8(unsigned crc, unsigned char const *data, unsigned int len)
{
    if (!data)
        return 0;
    crc &= 0xff;
    unsigned char const *end = data + len;
    while (data < end)
        crc = crc8_table[crc ^ *data++];
    return crc;
}
int __attribute((section(".main"))) __attribute__((__noipa__))  __attribute__((optimize("O0"))) main(void)
{   
    activate_crc_clock();

    enum {
        CrcAddress = 0x40023000,
    };

    typedef struct CrcRegisterMap {
        unsigned int crc_dr;
        unsigned int crc_idr;
        unsigned int crc_cr;
    } CrcRegisterMap_t;

    CrcRegisterMap_t* crc_regs = (CrcRegisterMap_t*) CrcAddress;
    WRITE_REGISTER(&crc_regs->crc_cr, READ_REGISTER(&crc_regs->crc_cr) | 1);

    unsigned char n[1] = {0x42};
    
    unsigned crc0 = crc8(0, (void*)0, 0);
    unsigned int crc2 = crc8(crc0, n, 1);
    unsigned char n1[2] = {0x42, 0xCF};
    unsigned int crc3 = crc8(crc2, n1, 2);

    asm("bkpt");

    return 0;
}
